...
comment on table access_log is 'サービス要求履歴';
comment on column access_log.access_log_id is '固有番号';
comment on column access_log.user_id is 'ユーザID';
comment on column access_log.user_name is 'ユーザー名';
comment on column access_log.client_ip is 'リクエストIP';
comment on column access_log.request_uri is 'URI';
comment on column access_log.parameters is 'リクエストParamter';
...
comment on column access_log.insert_date is '登録';
